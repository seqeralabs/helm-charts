...
- **BREAKING**: Removed `seqera.images.image` and `seqera.images.cloudProviderOverride` helpers.
  Callers should use `common.images.image` from the bitnami/common library chart directly.
- **BREAKING**: Removed the `cloudProviderImageKey` parameter from `seqera.initContainers.waitFor*`
  helpers. The init containers now always resolve their image via `common.images.image` and no
  longer honor `global.azure.images` overrides.
...
- `seqera.initContainers.waitForRedis` no longer prints the Redis password in plain text in the wait-loop log message; it now reports only whether auth is set
...
- `seqera.initContainers.waitForRedis` now sets `REDISCLI_TLS=1` automatically when `redis.enableTls` is true

## [2.0.1] - 2026-03-27

### Added

- Add `extraEnv` and `extraVolumeMounts` support to all `seqera.initContainers.waitFor*` helpers, allowing callers to mount CA certificates or inject additional environment variables into init containers
- Add `MYSQL_EXTRA_ARGS` env var interpolation to `seqera.initContainers.waitForMySQL` command, enabling TLS flags such as `--ssl-ca` and `--ssl-mode` to be passed at runtime

### Changed

- User-supplied `initContainers` now render before the built-in `waitFor*` init containers in all deployments, enabling use cases such as fetching CA certificates before dependency checks run
