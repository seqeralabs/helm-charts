...
- Bumped `mcp` subchart to `0.7.3`.
- Bumped `studios` subchart to `1.7.3`.

## [1.0.3] - 2026-09-21

### Changed

- Moved `TOWER_AUTH_COOKIE_DOMAIN` and `TOWER_AGENT_BACKEND_URL` (rendered when `agent-backend` is
  enabled) from the backend `ConfigMap` to the shared backend/cron `ConfigMap`, so the cron pod
...
- Removed the `portal-web` chart from the repo, which was getting packaged as part of platform.
...
- Bumped the `studios` subchart to `1.7.2`, which fixes the managed Redis password secret being
  written and looked up under different keys (`redis-password` vs `CONNECT_REDIS_PASSWORD`),
  causing `helm upgrade` to fail to find the existing password.
...
- Updated subchart `appVersion`s: `agent-backend` to `1.14.0`, `mcp` to `1.5.0`, and
  `studios` to `0.12.0`.
...
- Allow image `tag` fields to be specified as either a string or a number in `values.schema.json`
  across the `platform` chart and every subchart, so an unquoted numeric tag (for example
  `tag: 8.4`) no longer fails schema validation.
- Bumped subchart dependency versions: `mcp` to `0.6.x`, `portal-web` to `0.5.x`, `studios` to
  `1.6.x`, and `wave` to `0.4.x`.
