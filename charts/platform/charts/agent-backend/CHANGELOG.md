...
- **BREAKING**: The agent-backend Deployment is now pinned to a single replica with a `Recreate`
  ...
  `extraOptionsSpec` (they are silently ignored). Any values that previously set
  `extraOptionsSpec.replicas` or `extraOptionsSpec.strategy` should be removed.

## [1.4.2] - 2026-09-14

### Changed

- Updated `appVersion` to `1.14.1`.
...
- `AGENT_BACKEND_URL` in the ConfigMap, derived from `global.agentBackendDomain`
  (for example `https://ai-api.example.com`).
