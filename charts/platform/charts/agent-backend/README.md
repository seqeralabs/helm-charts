...
By default the chart selects the application images defined in the `appVersion` field of the `Chart.yaml` file, currently set as `2.0.0`.
...
| openaiApiKey | string | `""` | OpanAI API key. Define the value as a String or a Secret, not both at the same time |
| openaiApiKeyExistingSecretName | string | `""` | Name of an existing Secret containing the OpenAI API key. Note: the Secret must already exist in the same namespace at the time of deployment |
| openaiApiKeyExistingSecretKey | string | `"OPENAI_API_KEY"` | Key in the existing Secret containing the OpenAI API key |
