...
![Version: 1.5.0](https://img.shields.io/badge/Version-1.5.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.14.1](https://img.shields.io/badge/AppVersion-1.14.1-informational?style=flat-square)
...
By default the chart selects the application images defined in the `appVersion` field of the `Chart.yaml` file, currently set as `1.14.1`.
...
| extraOptionsSpec | object | `{}` | Extra options to place under .spec (e.g. revisionHistoryLimit, etc). Evaluated as a template. Note: `replicas`, `strategy`, and `selector` are always managed by the chart and cannot be overridden here. The agent-backend deployment MUST NOT be replicated, not even during a rolling deployment, so it is pinned to `replicas: 1` with a `Recreate` strategy. |
