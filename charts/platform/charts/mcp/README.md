...
- (Default) To authenticate against Seqera Platform, configure the chart's `.micronautEnvironments` with `oauth-platform`:
  * This configures Platform to become the OIDC provider for MCP.
  * When the MCP chart is deployed as a subchart of the Platform chart, the OIDC initial token is automatically retrieved from the Platform secret.
- To use a custom OAuth provider instead of Platform, configure the chart's `.micronautEnvironments` with `oauth`:
  * Set `oauth.issuerUrl`, `oauth.clientId`, and `oauth.clientSecretString` (or `oauth.clientSecretExistingSecretName`) values with the credentials to a custom OAuth provider.
...
| oauth.clientId | string | `""` | OAuth client ID for MCP to authenticate with a custom OAuth server. Not needed if using Seqera Platform as the OAuth provider (by setting 'oauth-platform' in `.micronautEnvironments`), as Platform automatically creates an internal client for MCP with the correct permissions. This is required when 'oauth' is set in `.micronautEnvironments` to authenticate with a custom OAuth provider instead of Platform. |
