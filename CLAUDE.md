...
- **values.schema.json is autogenerated**: Do NOT hand-edit `values.schema.json`. It's regenerated
  from `values.yaml` by the `helm-schema` pre-commit hook. After changing defaults/descriptions in
  `values.yaml`, just run `pre-commit run --all-files` (or let CI/the git hook do it) instead of
  manually editing the schema.
- **Full unittest runs are slow and already run on commit**: `make -C charts/platform test` /
  `helm unittest charts/platform/` exercise the whole chart (1000+ tests) and take over a minute.
  prek (this repo's pre-commit runner) already runs the full suite on every commit, so there's no
  need to run the whole thing yourself first. While iterating, target only the template(s) you're
  changing: `helm unittest -f 'tests/deployment_test.yaml' charts/platform/charts/mcp/` (glob
  patterns work too, e.g. `-f 'tests/*mcp*'`). Reach for the full `make test` run mainly as a final
  check before committing, or when a change could ripple into other templates (shared helpers,
  `_helpers.tpl`, common library bumps).
